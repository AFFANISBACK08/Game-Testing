...
function drawPitch() {
    // Deeper mowed-turf gradient per stripe instead of flat fill, so the
    // grass reads as real turf rather than a flat color bar.
    ...
    for (let i = 0; i < 10; i++) {
        ...
        if (dark) {
            ...
            g.addColorStop(0.5, '#217a45');
            g.addColorStop(1, '#1a6b3b');
        } else {
            g.addColorStop(0, '#2fb86a');
            g.addColorStop(0.5, '#29a45e');
            g.addColorStop(1, '#238f51');
        }
        ctx.fillStyle = g;
        ctx.fillRect(25 + i * stripeWidth, 0, stripeWidth, canvas.height);
    }

    // Crisp broadcast-style pitch markings with a soft glow so they read
    // clearly against the turf without looking neon.
    ctx.save();
    ...
    // Warm floodlight wash spilling in from the stadium above, tying the
    // pitch visually to the menu's "Matchday" lighting instead of a flat
    // center-glow.
    ctx.save();
    ctx.globalCompositeOperation = 'lighter';
    const fl1 = ctx.createRadialGradient(60, -40, 10, 60, -40, 420);
    fl1.addColorStop(0, 'rgba(255,215,106,0.05)');
    fl1.addColorStop(1, 'rgba(255,215,106,0)');
    ctx.fillStyle = fl1;
    ctx.fillRect(0, 0, canvas.width, canvas.height);
    const fl2 = ctx.createRadialGradient(840, -40, 10, 840, -40, 420);
    fl2.addColorStop(0, 'rgba(255,215,106,0.05)');
    fl2.addColorStop(1, 'rgba(255,215,106,0)');
    ctx.fillStyle = fl2;
    ctx.fillRect(0, 0, canvas.width, canvas.height);
    ctx.restore();

    // Vignette so the edges recede and the on-pitch action reads as the focus.
    const vg = ctx.createRadialGradient(450, 300, 200, 450, 300, 520);
...
function drawActiveIndicator(p, labelText, colorHex) {
    ...
    ctx.restore();
    ctx.fillStyle = colorHex;
    ctx.font = (typeof Theme !== 'undefined') ? Theme.display(15) : '900 13px Outfit, sans-serif';
...
function drawScoreboard() {
    ...
    let teamAFlagTeam = null;
    let teamBFlagTeam = null;
    ...
    if (tournamentMode && tournamentPendingMatch) {
        ...
        if (match.teamA && match.teamB) {
            if (match.teamA.id === playerTeamId) {
                teamAName = match.teamA.name; teamAColor = match.teamA.color; teamAFlagTeam = match.teamA;
                teamBName = match.teamB.name; teamBColor = match.teamB.color; teamBFlagTeam = match.teamB;
            } else {
                teamAName = match.teamB.name; teamAColor = match.teamB.color; teamAFlagTeam = match.teamB;
                teamBName = match.teamA.name; teamBColor = match.teamA.color; teamBFlagTeam = match.teamA;
            }
        }
    }
    ...
    // Main scoreline strip — kit-tag panel with clipped corners on both ends,
    // gold hairline, Bebas Neue score digits.
    if (T && typeof drawKitPanel === 'function') {
        ctx.save();
        drawKitPanel(120, 8, 660, 62, { clip: 16, fill: 'rgba(9,13,22,0.82)', border: 'rgba(255,215,106,0.35)' });
        ctx.restore();
    } else {
    ...
    ctx.textAlign = 'right';
    ctx.font = T ? T.body(12, 700) : '800 13px Outfit, sans-serif';
    ctx.fillStyle = teamAColor;
    let shortA = teamAName.length > 12 ? teamAName.slice(0,12)+'..' : teamAName;
    if (teamAFlagTeam && typeof drawTeamFlag === 'function') {
        const fw = drawTeamFlag(teamAFlagTeam, 320, 32, 12, 'left');
        ctx.textAlign = 'left';
        ctx.fillText(shortA, 320 + fw, 32);
        ctx.textAlign = 'right';
    } else {
        ctx.fillText(shortA, 390, 32);
    }
    ctx.font = T ? T.display(30) : '900 28px Outfit, sans-serif';
    ctx.fillStyle = teamAColor;
    ctx.shadowColor = teamAColor;
    ctx.shadowBlur = 8;
    ctx.fillText(score.red, 420, 55);
    ctx.shadowBlur = 0;

    ctx.textAlign = 'center';
    ctx.fillStyle = T ? T.gold : 'rgba(255,255,255,0.4)';
    ctx.font = T ? T.display(15) : '700 14px Outfit, sans-serif';
    ...
    if (teamBFlagTeam && typeof drawTeamFlag === 'function') {
        ctx.fillText(shortB, 480, 32);
        const bw = ctx.measureText(shortB).width;
        drawTeamFlag(teamBFlagTeam, 480 + bw + 4, 32, 12, 'left');
    } else {
        ctx.fillText(shortB + ' ', 480, 32);
    }
    ctx.font = T ? T.display(30) : '900 28px Outfit, sans-serif';
    ctx.fillStyle = teamBColor;
    ctx.shadowColor = teamBColor;
    ctx.shadowBlur = 8;
    ctx.fillText(score.blue, 470, 55);
    ctx.shadowBlur = 0;

    // Match clock chip
    if (T && typeof drawKitPanel === 'function') {
        drawKitPanel(400, 72, 100, 28, { clip: 8, fill: 'rgba(9,13,22,0.88)', border: 'rgba(255,215,106,0.3)' });
    } else {
    ...
    ctx.fillStyle = matchClock <= 5 ? '#ff5252' : (T ? T.gold : '#f1c40f');
    ctx.font = T ? T.display(20) : '800 18px Outfit, sans-serif';
    ctx.textAlign = 'center';
    ctx.fillText(timeStr, 450, 94);
    ctx.fillStyle = T ? T.chalkDim : 'rgba(255,255,255,0.5)';
    ctx.font = T ? T.body(9, 700) : '600 9px Outfit, sans-serif';
    ...
    if (gameMode === 'pve' && !tournamentMode) {
        if (T && typeof drawKitPanel === 'function') {
            drawKitPanel(15, 15, 80, 28, { clip: 8, fill: 'rgba(9,13,22,0.88)', border: 'rgba(255,215,106,0.3)' });
        } else {
            ctx.fillStyle = 'rgba(15,20,25,0.85)';
            ctx.beginPath();
        ...
        ctx.font = T ? T.body(13, 700) : '700 14px Outfit, sans-serif';
        ctx.textAlign = 'center';
...
function drawGkTimerUI() {
    if (gkTimer > 0 && ball.owner && ball.owner.isGk) {
        ...
        if (T && typeof drawKitPanel === 'function') {
            drawKitPanel(canvas.width - 80, 15, 65, 50, { clip: 10, fill: 'rgba(9,13,22,0.88)', border: 'rgba(255,215,106,0.3)' });
        } else {
            ctx.fillStyle = 'rgba(15,20,25,0.85)';
            ctx.beginPath();
        ...
        ctx.fillStyle = seconds <= 2 ? '#ff5252' : (T ? T.gold : '#f1c40f');
        ctx.font = T ? T.display(26) : '900 24px Outfit, sans-serif';
        ctx.textAlign = 'center';
        ctx.fillText(seconds + 's', canvas.width - 47, 43);
        ctx.fillStyle = T ? T.chalk : '#ffffff';
        ctx.font = T ? T.body(10, 700) : '700 10px Outfit, sans-serif';
...
function drawDifficultySelect() {
    drawMenuBackground();
    ctx.save();
    drawSectionHeader('Select Difficulty', 450, 84, { eyebrow: 'Choose your challenge level', size: 40 });

    // ===== PERFECTLY CENTERED 3+2 GRID =====
    // Row 1: 3 buttons (EASY, MEDIUM, HARD) – centered
    // Row 2: 2 buttons (ELITE, WORLD CLASS) – centered below
    const levels = [
        { key: 'EASY', label: 'EASY', sub: 'Casual Play', color: Theme.green, x: 200, y: 170 },
        { key: 'MEDIUM', label: 'MEDIUM', sub: 'Balanced', color: Theme.gold, x: 450, y: 170 },
        { key: 'HARD', label: 'HARD', sub: 'Expert', color: Theme.red, x: 700, y: 170 },
        { key: 'ELITE', label: 'ELITE', sub: 'Pro Level', color: '#c084fc', x: 325, y: 290 },
        { key: 'WORLD_CLASS', label: 'WORLD CLASS', sub: 'Ultimate', color: '#5eead4', x: 575, y: 290 }
    ];
    ...
    levels.forEach((lvl) => {
        ...
        const w = 140, h = 90;
        const x = lvl.x - w / 2, y = lvl.y;
        drawChoiceCard(x, y, w, h, lvl.label, lvl.sub, lvl.color, isSelected);
        window._difficultyBtns.push({ x, y, w, h, key: lvl.key });
    });

    drawBackPill(350, 410, 200, 45);
    window._diffBackBtn = { x: 350, y: 410, w: 200, h: 45 };

    ctx.fillStyle = Theme.chalkDim;
    ctx.font = Theme.body(12, 600);
    ctx.textAlign = 'center';
...
function drawPauseMenu() {
    ...
    ctx.textAlign = 'center';
    drawSectionHeader('Paused', 450, 210, { size: 44, tick: false });

    drawKitPanel(350, 235, 200, 50, { fill: `${Theme.green}22`, border: Theme.green });
    ctx.fillStyle = Theme.green;
    ctx.font = Theme.body(20, 700);
    ctx.fillText('▶ RESUME', 450, 267);

    drawKitPanel(350, 295, 200, 50, { fill: `${Theme.red}22`, border: Theme.red });
    ctx.fillStyle = Theme.red;
    ctx.font = Theme.body(20, 700);
    ctx.fillText('🏠 MAIN MENU', 450, 327);

    ctx.fillStyle = Theme.chalkDim;
    ctx.font = Theme.body(13, 600);
    ctx.fillText('SOUND CONTROLS', 450, 370);

    const musicColor = SoundManager.musicEnabled ? Theme.green : Theme.red;
    drawKitPanel(330, 385, 110, 35, { clip: 8, fill: `${musicColor}22`, border: musicColor });
    ctx.fillStyle = musicColor;
    ctx.font = Theme.body(15, 700);
    ctx.fillText(`🎵 ${SoundManager.musicEnabled ? 'ON' : 'OFF'}`, 385, 408);

    const sfxColor = SoundManager.sfxEnabled ? Theme.green : Theme.red;
    drawKitPanel(460, 385, 110, 35, { clip: 8, fill: `${sfxColor}22`, border: sfxColor });
    ctx.fillStyle = sfxColor;
    ctx.font = Theme.body(15, 700);
    ctx.fillText(`🔊 ${SoundManager.sfxEnabled ? 'ON' : 'OFF'}`, 515, 408);

    ctx.fillStyle = Theme.chalkDim;
    ctx.font = Theme.body(10, 500);
...
function drawStatsScreen() {
    drawMenuBackground();
    ctx.save();
    drawSectionHeader('Statistics', 450, 58, { size: 34 });
    ...
    const colors = [Theme.green, Theme.gold, Theme.red, '#c084fc', '#5eead4'];
    ...
    difficulties.forEach((diff, idx) => {
        ...
        drawKitPanel(x, y, cardWidth, cardHeight, { clip: 12, border: colors[idx], lineWidth: 2 });
        ...
        lines.forEach((line, i) => {
            ...
            ctx.fillText(line, centerX, y + 62 + i * 22);
        });
    });

    drawBackPill(350, 500, 200, 45);
...
function drawTournamentMenu() {
    drawMenuBackground();
    ctx.save();
    drawSectionHeader('Tournament', 450, 108, { size: 52, eyebrow: 'Select tournament format' });
    ...
    formats.forEach((fmt, idx) => {
        ...
        const x = 450 - 150, y = fmt.y - 15, w = 300, h = 65;
        drawChoiceCard(x, y, w, h, fmt.label, fmt.desc, Theme.gold, isSelected, { titleY: 0.42, subY: 0.72 });
        window._tournamentFormatBtns = window._tournamentFormatBtns || [];
        window._tournamentFormatBtns[idx] = { x, y, w, h, size: fmt.size };
    });

    drawKitPanel(300, 380, 300, 55, { clip: 14, fill: `${Theme.green}22`, border: Theme.green, glow: Theme.green });
    ctx.textAlign = 'center';
    ctx.fillStyle = Theme.green;
    ctx.font = Theme.display(26);
    ctx.fillText('▶ START TOURNAMENT', 450, 418);
    window._tournamentStartBtn = { x: 300, y: 380, w: 300, h: 55 };

    drawKitPanel(350, 450, 200, 35, { clip: 8, fill: 'rgba(255,255,255,0.04)', border: 'rgba(255,255,255,0.18)' });
    ctx.fillStyle = Theme.chalkDim;
    ctx.font = Theme.body(14, 700);
    ctx.fillText('← BACK', 450, 473);
...
function draw() {
    try {
        ...
        if (currentState === 'MENU') {
            ...
            ctx.textAlign = 'center';
            ctx.fillStyle = Theme.chalkDim;
            ctx.font = Theme.body(13, 700);
            ctx.fillText('WELCOME TO THE PITCH', 450, 62);
            ctx.fillStyle = Theme.chalk;
            ctx.font = Theme.display(74);
            ctx.shadowColor = Theme.gold;
            ctx.shadowBlur = 22;
            ctx.fillText('PRO STRIKER', 450, 128);
            ctx.shadowBlur = 0;
            ...
            const options = [
                { key: '01', label: '1 VS 1 MATCH', y: 230, color: Theme.red },
                { key: '02', label: 'VS COMPUTER', y: 285, color: Theme.blue },
                { key: '03', label: 'INSTRUCTIONS', y: 340, color: '#ff9f43' },
                { key: '04', label: 'SETTINGS', y: 395, color: '#c084fc' },
                { key: '05', label: 'STATS', y: 450, color: '#5eead4' },
                { key: '06', label: 'TOURNAMENT', y: 505, color: Theme.gold }
            ];
            for (let opt of options) {
                drawNavButton(280, opt.y - 28, 340, 44, opt.key, opt.label, opt.color);
            }
            ctx.save();
            ctx.fillStyle = Theme.chalkDim;
            ctx.font = Theme.body(11, 600);
            ctx.textAlign = 'right';
